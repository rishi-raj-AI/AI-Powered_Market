...
def deliver_event(db: Session, event: NotificationEvent) -> int:
    ...
    if messaging is None:
        event.status = "pending" if settings.FCM_PROJECT_ID else "in_app_only"
        return 0
    ...
    for device in devices:
        try:
            messaging.send(messaging.Message(notification=messaging.Notification(title=event.title, body=event.body), data=data, token=device.token))
            sent += 1
    ...
    if invalid_tokens:
        for device in devices:
            if device.token in invalid_tokens:
...
def flush_pending(db: Session, limit: int = 100) -> dict[str, int]:
    events = db.scalars(select(NotificationEvent).where(NotificationEvent.status == "pending").order_by(NotificationEvent.created_at).limit(limit)).all()
    delivered = 0
    for event in events:
