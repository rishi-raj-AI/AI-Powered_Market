...
def _location_read(location: DeliveryLocation) -> DeliveryLocationRead:
    return DeliveryLocationRead(id=location.id,delivery_id=location.delivery_id,latitude=location.latitude,longitude=location.longitude,accuracy_m=location.accuracy_m,heading_deg=location.heading_deg,speed_mps=location.speed_mps,recorded_at=location.recorded_at)
...
def _tracking_context(db: Session, order_id: uuid.UUID, user: User):
    ...
    return order, delivery, db.get(Store, order.store_id), db.get(Address, order.address_id)


def _limit_route(user: User) -> None:
    try:
        rate_limiter.enforce("maps-route", str(user.id), limit=20, window_seconds=60)
...
def record_delivery_location(delivery_id: uuid.UUID,payload: DeliveryLocationCreate,db: Session = Depends(get_db),rider: User = Depends(require_roles(UserRole.DELIVERY))):
    ...
    now = datetime.now(timezone.utc);recorded_at = payload.recorded_at.astimezone(timezone.utc)
    ...
    location = DeliveryLocation(delivery_id=delivery.id,delivery_partner_id=rider.id,latitude=payload.latitude,longitude=payload.longitude,accuracy_m=payload.accuracy_m,heading_deg=payload.heading_deg,speed_mps=payload.speed_mps,recorded_at=recorded_at)
    db.add(location);db.commit();db.refresh(location);return _location_read(location)


@router.get("/orders/{order_id}/tracking", response_model=OrderTrackingRead)
def order_tracking(order_id: uuid.UUID,db: Session = Depends(get_db),user: User = Depends(get_current_user)):
    ...
    age_seconds = max(0, int((datetime.now(timezone.utc) - latest.recorded_at).total_seconds())) if latest is not None else None
    return OrderTrackingRead(order_id=order.id,order_number=order.order_number,order_status=order.status,delivery_id=delivery.id if delivery else None,delivery_status=delivery.status if delivery else None,tracking_active=active,store=TrackingPoint(latitude=float(store.latitude) if store and store.latitude is not None else None,longitude=float(store.longitude) if store and store.longitude is not None else None,label=store.name if store else None),customer=TrackingPoint(latitude=address.latitude if address else None,longitude=address.longitude if address else None,label=address.landmark if address else None),rider=_location_read(latest) if latest else None,rider_location_age_seconds=age_seconds)
...
def order_route(order_id: uuid.UUID,db: Session = Depends(get_db),user: User = Depends(get_current_user)):
    ...
    return RouteRead(available=True,provider="google",origin=origin,destination=destination,distance_meters=result.distance_meters,duration_seconds=result.duration_seconds,encoded_polyline=result.encoded_polyline)
