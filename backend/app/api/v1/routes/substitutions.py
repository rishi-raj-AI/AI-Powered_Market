...
def substitutions(
    ...
    base_listing = db.get(StoreProduct, listing_id)
    if base_listing is None:
        raise HTTPException(status_code=404, detail='Listing not found')
    base_product = db.get(Product, base_listing.product_id)
    ...
    rows = db.execute(
        ...
        .where(
            StoreProduct.store_id == base_listing.store_id,
            StoreProduct.id != base_listing.id,
    ...
    ranked: list[SubstitutionRead] = []
    for listing, product in rows:
        score = _substitution_score(base_listing.price, listing.price, bool(base_product.brand and product.brand and base_product.brand.casefold() == product.brand.casefold()))
        ranked.append(SubstitutionRead(
            ...
            unit=product.unit,
            price=listing.price,
            price_delta=listing.price - base_listing.price,
            score=score,
        ))
