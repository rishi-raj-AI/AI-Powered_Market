...
class _StoreScreenState extends State<StoreScreen> {
  ...
  Future<void> load() async {
    try {
      ...
      try { nextPreparation = await CommerceIntelligenceApi.preparationEstimate(widget.store.id); }
      catch (exception) { nextPreparationError = '$exception'.replaceFirst('Exception: ', ''); }
      if (!mounted) return;
      ...
      if (mounted) setState(() { loading = false; error = '$exception'.replaceFirst('Exception: ', ''); });
    }
  }
  ...
  Future<bool> _prepareCartForStore() async {
    ...
    final replace = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(
      title: const Text('Start a new cart?'),
      content: const Text('GaonOne keeps one store per order so delivery and inventory stay reliable. Your current cart is from another store.'),
      actions: [TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Keep current cart')), FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Clear and continue'))],
    ));
  ...
  Widget build(BuildContext context) {
    ...
    final visibleProducts = normalized.isEmpty ? products : products.where((p) => p.name.toLowerCase().contains(normalized) || p.unit.toLowerCase().contains(normalized)).toList();
    return Scaffold(
      appBar: AppBar(title: Text(widget.store.name)),
      body: loading ? const Center(child: CircularProgressIndicator()) : RefreshIndicator(
        onRefresh: load,
        child: ListView(padding: const EdgeInsets.all(16), children: [
          Text(widget.store.name, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w800)),
          const SizedBox(height: 4),
          Text(widget.store.landmark ?? widget.store.description ?? 'Local store'),
          StoreTrustCard(storeId: widget.store.id),
          ...
          ...visibleProducts.map((product) => Card(child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(product.name, style: const TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 3), Text('${product.unit} • ${product.stock} in stock'), if (product.mrp != null && product.mrp != product.price) Text('MRP ₹${product.mrp}', style: Theme.of(context).textTheme.bodySmall)])),
