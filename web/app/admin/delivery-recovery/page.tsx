...
import {DeliveryRecoverySubmission} from '@/lib/delivery-adapters';
...
export default function DeliveryRecovery(){
  ...
  useEffect(()=>{load()},[]);
  async function resolve(task:FailedDelivery,resolution:DeliveryRecoverySubmission['resolution']){setBusy(task.id);const payload:DeliveryRecoverySubmission={resolution};try{await api(`/admin/deliveries/${task.id}/resolve-failure`,{method:'POST',body:JSON.stringify(payload)});await load()}catch(e:any){setError(e.message)}finally{setBusy('')}}
