...
availableDeliveries:()=>api<Delivery[]>('/delivery/available'),myDeliveries:()=>api<Delivery[]>('/delivery/me'),claimDelivery:(id:string)=>api<Delivery>(`/delivery/${id}/claim`,{method:'POST'}),updateDelivery:(id:string,status:string)=>api<Delivery>(`/delivery/${id}/status`,{method:'PATCH',body:JSON.stringify({status})})};
