...
export function LocationMap({latitude,longitude,onChange,editable=false,height=280,zoom=16,markers=[],className=''}:Props){
  ...
  useEffect(()=>{
    ...
    g.maps.importLibrary('marker').then(({AdvancedMarkerElement}:any)=>{
      ...
      if(markers.length>1){const bounds=new g.maps.LatLngBounds();markers.forEach(p=>bounds.extend({lat:p.lat,lng:p.lng}));map.fitBounds(bounds,56)}
    }).catch(()=>{});
