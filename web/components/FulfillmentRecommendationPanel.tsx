...
export function FulfillmentRecommendationPanel({storeId}:{storeId:string}){
  ...
  async function check(){
    ...
    navigator.geolocation.getCurrentPosition(async position=>{
      try{const next=await fulfillmentRecommendation(storeId,position.coords.latitude,position.coords.longitude);setResult(next);setSelected(next.recommended_mode)}
  ...
  const options:FulfillmentMode[]=[];
  if(result){
    if(result.delivery_enabled&&result.delivery_serviceable&&result.store_open)options.push('delivery_now');
    if(result.pickup_enabled&&result.store_open)options.push('pickup_now');
    if(result.delivery_enabled&&result.delivery_serviceable&&!result.store_open)options.push('scheduled_delivery');
    if(result.pickup_enabled&&!result.store_open)options.push('scheduled_pickup');
    if(!options.length)options.push('unavailable');
  }
