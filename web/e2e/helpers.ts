...
export async function installApiMocks(page:Page,currentUser:MockUser=customer){
  ...
  await page.route('http://localhost:8000/api/v1/**',async route=>{
    ...
    if(path==='/stores/nearby')return route.fulfill({json:[{id:'store-nearby',merchant_id:approvedMerchant.id,village_id:village.id,service_area_id:serviceArea.id,name:'Niphad Daily Needs',slug:'niphad-daily-needs',landmark:'Niphad Bus Stand',delivery_enabled:true,pickup_enabled:true,is_active:true,is_open_now:true,distance_km:0.8}]});
    if(path==='/stores/store-nearby')return route.fulfill({json:{id:'store-nearby',merchant_id:approvedMerchant.id,village_id:village.id,service_area_id:serviceArea.id,name:'Niphad Daily Needs',slug:'niphad-daily-needs',landmark:'Niphad Bus Stand',opens_at:'08:00:00',closes_at:'21:00:00',delivery_enabled:true,pickup_enabled:true,is_active:true,is_open_now:true}});
    if(path==='/stores/store-nearby/products')return route.fulfill({json:[]});
    if(path==='/discovery/search')return route.fulfill({json:{query:url.searchParams.get('q'),latitude:20.0778,longitude:74.1118,radius_km:20,stores:[],categories:[],products:[{listing_id:'listing-rice',product_id:'product-rice',store_id:'store-nearby',store_name:'Niphad Daily Needs',name:'Kolam Rice',brand:'Gaon Fresh',unit:'1 kg',price:'72.50',distance_km:0.8,match_score:4.9}]}});
    if(path==='/stores'&&method==='GET')return route.fulfill({json:[]});
